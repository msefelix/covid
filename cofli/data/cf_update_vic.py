...
def vic_by_location(root_folder=bucket):
    ...
    fetch_url_to_gcs(fs, vic_active_by_gov, f"{root_folder}/data/vic/gov/vic_active_by_gov_{today}.csv")
    ...
    fetch_url_to_gcs(fs, vic_active_by_post, f"{root_folder}/data/vic/gov/vic_active_by_post_{today}.csv")
...
def add_a_csv(root_folder=bucket, filename='post'):
    df_new = pd.read_csv(f"{root_folder}/data/vic/gov/vic_active_by_{filename}_{today}.csv")
    df_new = process_a_post(df_new)

    df_old = pd.read_parquet(f"{root_folder}/data/vic/cases_{filename}.parquet")

    df = pd.concat([df_old, df_new], axis=0, ignore_index=True).drop_duplicates()
    df.to_parquet(f"{root_folder}/data/vic/cases_{filename}.parquet")
